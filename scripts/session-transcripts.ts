 ...
 *   --analyze      Spawn repi subagents to analyze each transcript file
...
function runSubagent(prompt: string, cwd: string): Promise<{ success: boolean }> {
	return new Promise((resolve) => {
		const child = spawn("repi", ["--mode", "json", "--tools", "read,write", "-p", prompt], {
			cwd,
		...
		child.on("error", (err) => {
			console.error(chalk.red(`  Failed to spawn repi: ${err.message}`));
...
async function main() {
	...
	const sessionsBase = join(homedir(), ".repi/agent/sessions");
	...
	if (!analyzeFlag) {
		console.log("\nRun with --analyze to spawn repi subagents for pattern analysis.");
		return;
	}

	// Find AGENTS.md files to compare against
	const globalAgentsMd = join(homedir(), ".repi/agent/AGENTS.md");
