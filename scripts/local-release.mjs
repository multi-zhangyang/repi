...
function prepareOutputDirectory(options, repoRoot) {
	if (!options.outDir) {
		return mkdtempSync(join(tmpdir(), "repi-local-release-"));
	}
...
function buildBunBinaryRelease(targetDirectory, archiveDirectory) {
	...
	const archiveName = platform.startsWith("windows-") ? `repi-${platform}.zip` : `repi-${platform}.tar.gz`;
	cpSync(join(binaryBuildDirectory, archiveName), join(archiveDirectory, archiveName));
	return platform;
}

function createRepiShim(installDirectory) {
	const binDirectory = join(installDirectory, "node_modules", ".bin");
	if (process.platform === "win32") {
		if (existsSync(join(binDirectory, "repi.cmd"))) {
			writeFileSync(join(installDirectory, "repi.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\repi.cmd" %*\r\n');
			writeFileSync(join(installDirectory, "repi.ps1"), '& "$PSScriptRoot/node_modules/.bin/repi.ps1" @args\n');
			return;
		}
		writeFileSync(join(installDirectory, "repi.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\repi.exe" %*\r\n');
		writeFileSync(join(installDirectory, "repi.ps1"), '& "$PSScriptRoot/node_modules/.bin/repi.exe" @args\n');
		return;
	}
	symlinkSync(join("node_modules", ".bin", "repi"), join(installDirectory, "repi"));
}
...
if (!options.skipInstall) {
	...
	createRepiShim(nodeInstallDirectory);
	...
	if (!options.skipBunInstall) {
		...
		createRepiShim(bunInstallDirectory);
	}
}
...
if (!options.skipInstall) {
	...
	console.log(`  ${join(outDir, `repi-${binaryPlatform}.${String(binaryPlatform).startsWith("windows-") ? "zip" : "tar.gz"}`)}`);
	console.log("\nRun the local Bun binary release from outside the repository:");
	console.log(`  ${join(binaryDirectory, String(binaryPlatform).startsWith("windows-") ? "repi.exe" : "repi")} --help`);

	console.log("\nIsolated npm install:");
	console.log(`  ${nodeInstallDirectory}`);
	console.log("\nRun the locally packed npm CLI from outside the repository:");
	console.log(`  ${join(nodeInstallDirectory, process.platform === "win32" ? "repi.cmd" : "repi")} --help`);
	...
	if (!options.skipBunInstall) {
		...
		console.log(`  ${join(bunInstallDirectory, process.platform === "win32" ? "repi.cmd" : "repi")} --help`);
	}
}
