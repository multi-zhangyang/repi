...
function staticContractChecks() {
	...
	checks.push(markerCheck("memory:vector-hard-eval", "scripts/reverse-agent/memory-vector-gate.mjs", ["repi-memory-vector-gate", "runtime:index-schema", "runtime:search-schema", "runtime:vector-rerank-used", "fixture:vector-rerank-negative"], []));
	checks.push(markerCheck("memory:vector-schema", "schemas/reverse-agent/memory-vector.schema.json", ["MemoryVectorIndexV1", "MemoryVectorSearchReportV1", "repi-local-hash-embedding-v1"], []));
	checks.push(markerCheck("memory:vector-fixture", "fixtures/reverse-agent/memory-vector.fixture.json", ["repi-memory-vector-fixture", "semantic-authz-rerank", "forbidden-cross-route-vector-leak", "quality-weighted-replay-boost"], []));
