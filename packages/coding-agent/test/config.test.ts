...
function createNpmPrefixInstall(template = "pi-prefix-"): { prefix: string; packageDir: string } {
	...
	const packageDir = join(scopeDir, "repi-coding-agent");
...
function createPnpmGlobalInstall(): { root: string; packageDir: string } {
	...
	const packageDir = join(root, "@mariozechner", "repi-coding-agent");
	mkdirSync(packageDir, { recursive: true });
	...
	setExecPath(
		join(
			root,
			".pnpm",
			"@mariozechner+repi-coding-agent@0.0.0",
			"node_modules",
			"@mariozechner",
			"repi-coding-agent",
			"dist",
...
function createYarnGlobalInstall(): { globalDir: string; packageDir: string } {
	...
	const packageDir = join(globalDir, "node_modules", "@mariozechner", "repi-coding-agent");
	mkdirSync(packageDir, { recursive: true });
	mkdirSync(binDir, { recursive: true });
	...
	process.env.PATH = `${binDir}${delimiter}${originalPath ?? ""}`;
	process.env.PI_PACKAGE_DIR = packageDir;
	setExecPath(join(globalDir, ".yarn", "@mariozechner", "repi-coding-agent", "dist", "cli.js"));
...
describe("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		setExecPath(
			"C:\\Users\\Admin\\Documents\\pnpm-repository\\global\\5\\.pnpm\\@earendil-works+repi-coding-agent@0.67.68\\node_modules\\@earendil-works\\repi-coding-agent\\dist\\cli.js",
		);
	...
	test("self-updates renamed packages from the current install prefix", () => {
		...
		const command = getSelfUpdateCommand("@mariozechner/repi-coding-agent", undefined, "@new-scope/pi");

		expect(command).toEqual({
			command: "npm",
			args: ["--prefix", prefix, "install", "-g", "--ignore-scripts", "--min-release-age=0", "@new-scope/pi"],
			display: `npm --prefix ${prefix} uninstall -g @mariozechner/repi-coding-agent && npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @new-scope/pi`,
			steps: [
				{
					command: "npm",
					args: ["--prefix", prefix, "uninstall", "-g", "@mariozechner/repi-coding-agent"],
					display: `npm --prefix ${prefix} uninstall -g @mariozechner/repi-coding-agent`,
				},
				{
					command: "npm",
					args: ["--prefix", prefix, "install", "-g", "--ignore-scripts", "--min-release-age=0", "@new-scope/pi"],
	...
	test("does not infer Windows npm custom prefixes from package paths", () => {
		const packageDir = "C:\\Users\\Admin\\npm prefix\\node_modules\\@earendil-works\\repi-coding-agent";
		process.env.PI_PACKAGE_DIR = packageDir;
	...
	test("self-updates renamed pnpm global installs by removing the old package first", () => {
		...
		expect(command).toEqual({
			...
			display:
				"pnpm remove -g @mariozechner/repi-coding-agent && pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @new-scope/pi",
			steps: [
				{
					command: "pnpm",
					args: ["remove", "-g", "@mariozechner/repi-coding-agent"],
					display: "pnpm remove -g @mariozechner/repi-coding-agent",
				},
				{
					command: "pnpm",
					args: ["install", "-g", "--ignore-scripts", "--config.minimumReleaseAge=0", "@new-scope/pi"],
	...
	test("self-updates renamed yarn global installs by removing the old package first", () => {
		...
		expect(command).toEqual({
			...
			display:
				"yarn global remove @mariozechner/repi-coding-agent && yarn global add --ignore-scripts @new-scope/pi",
			steps: [
				{
					command: "yarn",
					args: ["global", "remove", "@mariozechner/repi-coding-agent"],
					display: "yarn global remove @mariozechner/repi-coding-agent",
				},
				{
					command: "yarn",
					args: ["global", "add", "--ignore-scripts", "@new-scope/pi"],
	...
	test("self-updates renamed bun global installs by removing the old package first", () => {
		...
		expect(command).toEqual({
			...
			display:
				"bun uninstall -g @mariozechner/repi-coding-agent && bun install -g --ignore-scripts --minimum-release-age=0 @new-scope/pi",
			steps: [
				{
					command: "bun",
					args: ["uninstall", "-g", "@mariozechner/repi-coding-agent"],
					display: "bun uninstall -g @mariozechner/repi-coding-agent",
				},
				{
					command: "bun",
					args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@new-scope/pi"],
