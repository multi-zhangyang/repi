...
All keyboard shortcuts can be customized via `~/.repi/agent/keybindings.json`. Each action can be bound to one or more keys.
...
Create `~/.repi/agent/keybindings.json`:
...
On native Windows, `app.suspend` has no default binding because Windows terminals do not support Unix job control. If you bind it manually, repi shows a status message instead of suspending. In WSL, the normal Linux `ctrl+z`/`fg` behavior still applies.
