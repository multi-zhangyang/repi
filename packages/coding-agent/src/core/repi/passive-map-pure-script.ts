...
export function passiveMapScript(target?: string, depth?: number): string {
	...
	return [
		"set +e",
		`TARGET=${targetArg}`,
		'case "$TARGET" in http://*|https://*) ROOT="." ;; *) if [ -d "$TARGET" ]; then ROOT="$TARGET"; elif [ -f "$TARGET" ]; then ROOT="$(dirname "$TARGET")"; else ROOT="."; fi ;; esac',
		'echo "## context"',
		"pwd",
		'printf "target=%s\\n" "$TARGET"',
		'printf "root=%s\\n" "$ROOT"',
		...
		'echo "## target-stat"',
		'if [ -e "$TARGET" ]; then ls -la "$TARGET"; file "$TARGET" 2>/dev/null || true; if [ -f "$TARGET" ]; then sha256sum "$TARGET" 2>/dev/null || true; command -v checksec >/dev/null 2>&1 && checksec --file="$TARGET" 2>/dev/null | sed "s/^/[checksec] /" || true; command -v readelf >/dev/null 2>&1 && readelf -h "$TARGET" 2>/dev/null | sed "s/^/[elf-header] /" | head -40 || true; command -v strings >/dev/null 2>&1 && strings -n 6 "$TARGET" 2>/dev/null | grep -Eai "password|license|token|flag|secret|strcmp|memcmp|admin|debug|key" | head -80 | sed "s/^/[strings] /" || true; fi; else echo "target_missing=$TARGET"; fi',
		'echo "## file-inventory"',
