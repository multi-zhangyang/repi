...
export function passiveMapScript(target?: string, depth?: number): string {
	...
	return [
		...
		`find "$ROOT" -maxdepth ${maxDepth} -type f ( -path '*/.git/*' -o -path '*/node_modules/*' -o -path '*/dist/*' -o -path '*/build/*' -o -path '*/.repi/*' -o -path '*/.cache/*' -o -path '*/__pycache__/*' ) -prune -o -type f ( -perm -111 -o -name '*.so' -o -name '*.dll' -o -name '*.exe' -o -name '*.elf' -o -name '*.wasm' -o -name '*.apk' -o -name '*.ipa' -o -name 'crackme*' -o -name 'vuln' ) -print 2>/dev/null | sort | head -80 | while IFS= read -r f; do file "$f" 2>/dev/null | grep -E "ELF|PE32|Mach-O|Zip archive|Android package|Dalvik|WebAssembly" || true; done | head -120`,
		'echo "## pcap-candidates"',
		`find "$ROOT" -maxdepth ${maxDepth} -type f \\( -name '*.pcap' -o -name '*.pcapng' -o -name '*.cap' \\) -print 2>/dev/null | head -40`,
		"fi",
		'case "$TARGET" in http://*|https://*) echo "## http-baseline"; curl -k -sS -D - -o /dev/null --max-time 8 -A "Mozilla/5.0 REPI-passive-map" "$TARGET" 2>&1 | sed -n "1,80p"; curl -k -sS --max-time 6 -A "Mozilla/5.0 REPI-passive-map" "$TARGET/robots.txt" 2>&1 | sed "s/^/[robots] /" | head -40; printf "[http-note] head_probe_skipped=1 use_get_status=1\\n";; esac',
	].join("\n");
}
