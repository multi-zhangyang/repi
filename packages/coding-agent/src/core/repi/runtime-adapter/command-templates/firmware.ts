...
export function rootfsServiceMapCommandTemplate(mode: "native" | "fallback"): string {
	...
	const mid = [
		...
		'  if [ "$CAP_ACCOUNT" = "1" ] || [ "$CAP_SERVICE" = "1" ] || [ "$CAP_ELF" = "1" ]; then CAP_CONFIG=1; CAP_EXTRACT=1; fi',
		"fi",
		'if [ -f "$target" ]; then',
		'  if [ "$CAP_BINWALK" = "1" ] || [ "$CAP_EXTRACT" = "1" ]; then CAP_IMAGE=1;',
		'  elif file -b "$target" 2>/dev/null | grep -qiE "squashfs|u-boot|uimage|firmware|data|archive|filesystem"; then CAP_IMAGE=1;',
		'  elif strings -a -n 6 "$target" 2>/dev/null | grep -qiE "squashfs|uboot|u-boot|busybox|jffs2|yaffs|trx"; then CAP_IMAGE=1;',
		"  fi",
		"fi",
