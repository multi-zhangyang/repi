...
export function appendDomainLaneNativeRuntime(ctx: DomainLaneRuntimeCtx, add: AddFn): void {
	const {
		laneName,
		isNativeRoute,
		isAndroidRoute,
	...
	// reverse domain capture next for reverse-heavy runtime/proof lanes
	if (
		...
		const reverseNext = reverseDomainCaptureNextCommands({
			routeOrBlob: `${laneName} ${targetArg ?? ""} native domain lane`,
			target: effectiveTarget,
			includeGates: true,
		}).slice(0, 2);
		for (const command of reverseNext) {
			add("domain-lane-reverse-next", command, "reverse domain capture next");
		}
	}
}
