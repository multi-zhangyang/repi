...
export function appendLaneDomainNativeTriage(ctx: LaneDomainPackCtx): void {
	...
	if (isNativeRoute || (isPwnRoute && /triage|map|mitigation/.test(laneName))) {
		if (targetIsDirectory) {
			...
			add(
				...
				`find ${targetArg} -maxdepth 4 -type f -exec file {} \\; 2>/dev/null | tee ${runtimeMisc}/directory-file-map.txt | grep -E 'ELF|PE32|Mach-O|WebAssembly|script|Zip archive|Android package|pcap' | head -160`,
				"typed candidate map: binaries, scripts, archives, APKs and PCAPs",
			);
			add(
				"directory-triage-candidates",
				`awk -F: '/ELF|PE32|Mach-O|WebAssembly|script|Zip archive|Android package|pcap/ {print $1}' ${runtimeMisc}/directory-file-map.txt 2>/dev/null | head -80`,
