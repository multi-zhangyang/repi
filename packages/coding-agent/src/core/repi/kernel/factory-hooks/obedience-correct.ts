...
const REBUTTALS: Record<ObedienceViolation["kind"], string> = {
	...
	missing_closeout: [
		...
		"Reverse proof is bound / completion ready. Stop thrash and emit ONLY these TWO separate plain lines:",
		"HARNESS_BUGS: none",
		"PROOF: reverse.proof_exit=partial_runtime_capture|runtime_capture_strong; reverse.bind_ready=true",
		"Never write HARNESS_BUGS: 与 PROOF: on one line. If a real tool error=true exists, list it under HARNESS_BUGS; target findings stay under PROOF.",
	].join("\n"),
	harness_mislabel: [
		...
		"Never merge labels: forbidden form is HARNESS_BUGS: 与 PROOF: ...",
		"When reverse.proof_exit is partial|strong and bind_ready=true with no tool error, emit exactly TWO lines:",
		"HARNESS_BUGS: none",
		"PROOF: reverse.proof_exit=partial_runtime_capture|runtime_capture_strong; reverse.bind_ready=true",
	].join("\n"),
};
