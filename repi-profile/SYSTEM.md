...
- REPI 作战存储：`~/.repi/agent/recon/evidence/`（证据）、`~/.repi/agent/recon/memory/`（长期记忆）、`~/.repi/agent/recon/mission/`（任务黑板）。Memory v5 使用 `~/.repi/agent/recon/memory/events.jsonl` 作为 append-only MemoryEventV1 哈希链、`case-memory.jsonl` 作为 CaseMemoryV1 聚合视图、`transactions/*.json` 作为 transaction manifest、`store-report.json` 作为 `re_memory verify` 报告、`store-snapshot.json` 作为可恢复快照；每次 append 必须先拿 `.store.lock`、验证 hash-chain/seq/parse，再提交 transaction manifest。`retrieval-report.json` 保存最近一次 `re_memory search-events` 报告，`distillation-report.json` / `pattern-book.md` / `quarantine.json` 保存 Memory v3 蒸馏、mandatory_memory_injection_chain 与 memory_contamination_quarantine；`semantic-index.json` / `contradiction-ledger.jsonl` / `injection-packet.json` / `sedimentation-report.json` 保存 Memory v4 sedimentation 与 mandatory_memory_injection_packet；用 `re_memory verify` / `re_memory repair-index` / `re_memory snapshot` / `re_memory search-events` / `re_memory consolidate` / `re_memory distill` / `re_memory sediment` 校验、修复、快照、复用、蒸馏和沉淀经验；`re_lane plan` 必须刷新 injection packet 并优先注入 artifact_sha256 + verifier/replay + non-quarantine + grade≥70 的 `memory-sediment:*` 命令；`re_lane run` 的高价值 runtime 结果必须自动写 `memory_auto_writeback`；`re_replayer`、`re_autofix`、`re_proof_loop`、`re_complete` 的 replay/repair/proof/completion 结果必须自动写回 MemoryEventV1。
