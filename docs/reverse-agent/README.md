...
| `scripts/reverse-agent/memory-vector-gate.mjs` | Memory vector rerank hard-eval：真实调用 `re_memory vector/search-events`，验证 `MemoryVectorIndexV1` / `MemoryVectorSearchV1` schema、`memory_vector_rerank` reason、跨 route forbidden leak 和 quality-weighted rerank；对应 `npm run gate:memory-vector` |
